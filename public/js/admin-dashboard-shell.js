(function(){
  ...
  function build(){
    ...
    sidebar.innerHTML='<a class="mtd-shell-brand" href="admin.html"><img src="images/logo-agro.jpeg" alt="Logo"><span><strong>MYCELIUM</strong><small>TECH DIGITAL</small></span></a><nav class="mtd-shell-nav">'+links.map(([href,icon,label,role])=>'<a href="'+href+'"'+(page===href?' class="active"':'')+(role?' data-mtd-admin-link hidden':'')+'><i data-lucide="'+icon+'"></i>'+label+'</a>').join('')+'</nav><section class="mtd-shell-system"><small>État du système</small><strong>Opérationnel</strong><p>Données et photos protégées dans Supabase.</p></section><div class="mtd-shell-footer">Cultiver l’avenir, naturellement.<br><strong>Mycelium Tech Digital</strong></div>';
    const top=document.createElement('header');top.className='mtd-shell-topbar';
    top.innerHTML='<button class="mtd-shell-menu" type="button" aria-label="Ouvrir le menu"><i data-lucide="menu"></i></button><label class="mtd-shell-search"><i data-lucide="search"></i><input type="search" placeholder="Rechercher une boîte, souche, matrice…" aria-label="Rechercher"></label><span class="mtd-shell-spacer"></span><button class="mtd-shell-icon" type="button" title="Apparence"><i data-lucide="sun"></i></button><button class="mtd-shell-icon" id="mtd-shell-bell" type="button" title="Demandes de suppression" aria-label="Demandes de suppression" hidden><i data-lucide="bell"></i><span class="mtd-shell-badge" id="mtd-shell-badge" hidden>0</span></button><div class="mtd-shell-profile"><img src="images/logo-agro.jpeg" alt=""><span><strong id="mtd-shell-name">Utilisateur</strong><small id="mtd-shell-role">Connexion…</small></span></div><button class="mtd-shell-logout" type="button" title="Déconnexion" aria-label="Déconnexion"><i data-lucide="chevron-down"></i></button>';
